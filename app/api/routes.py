...
def agent_status(user: Annotated[UserAccount, Depends(current_user)]):
    ...
    return {
        ...
        "agentFramework": {"active": "event_driven_blackboard"},
        "finetunedModel": finetuned_model_status(settings),
        "agents": [
            {"name": "CoordinatorAgent", "status": "READY", "description": "消费事件、维护 flow、安全门槛和下一批 Agent 命令"},
        ...
        "loop": {
            "type": "event-driven-blackboard",
            "maxSteps": settings.agent_runtime_max_events,
            "scheduler": "event-mailbox-dispatcher",
        },
        "collaboration": {
            "scheduler": "coordinator-command-batches",
            "state": "typed-versioned-blackboard",
            "messageBus": "request-scoped async event mailbox with batched durable journal",
            "fixedWorkflow": False,
            "parallelism": "bounded asyncio.gather batches",
            "checkpoint": "auto-resumable MySQL checkpoint and idempotent event journal",
