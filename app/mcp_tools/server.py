...
def mindbridge_excel_report(report_id: int) -> str:
    """Write one psychological risk report into the 心理ai Excel ledger."""
    create_schema()
    db = SessionLocal()
    try:
...
def mindbridge_case_create(report_id: int) -> str:
    """Create or return the active 心理ai risk case for one psychological report."""
    create_schema()
    db = SessionLocal()
    try:
        report = db.get(PsychologicalReport, report_id)
...
def mindbridge_alert_send(case_id: int) -> str:
    """Send or record the counselor alert for one 心理ai risk case."""
    create_schema()
    db = SessionLocal()
    try:
...
def mindbridge_alert_ack(case_id: int, actor: str, note: str = "") -> str:
    """Mark a 心理ai risk case as acknowledged by a counselor or administrator."""
    create_schema()
    db = SessionLocal()
    try:
...
def mindbridge_case_note_add(case_id: int, actor: str, note: str) -> str:
    """Append a follow-up note to a 心理ai risk case."""
    create_schema()
    db = SessionLocal()
    try:
