...
class RedisShortTermMemoryStore:
    ...
    def append(
        ...
        *,
        ...
        limit = max(1, max_messages or self.settings.redis_memory_max_messages)
        try:
...
def compact_history_for_prompt(
    ...
    result = compact_history_with_trace(history, settings, current_input, compaction_id="compat")
    return list(result.messages), result.summary


def compact_history_with_trace(
    history: list[AiMessage],
    settings: MemoryCompactionSettings,
    current_input: str = "",
    *,
    ...
    """Compact history and return deterministic lifecycle/audit metadata."""
    ...
    if not getattr(settings, "memory_compaction_enabled", True) or len(sanitized) <= recent_count:
        return MemoryCompactionResult(
            ...
            retained_message_count=len(sanitized),
            compacted=False,
            summary_hash=_summary_hash(brief),
        )
