...
def _remove_legacy_confidence_column(sheet) -> None:
    """Migrate an existing risk ledger away from the removed confidence column."""
    headers = [cell.value for cell in sheet[1]] if sheet.max_row >= 1 else []
    if "confidence" in headers:
        sheet.delete_cols(headers.index("confidence") + 1, 1)
...
class ToolOrchestrationService:
    ...
    def write_excel(self, report: PsychologicalReport) -> ExcelRecord:
        ...
        with EXCEL_WRITE_LOCK:
            if path.exists():
                ...
                workbook = Workbook()
                sheet = workbook.active
                sheet.title = "心理ai Risk Ledger"
                sheet.append(["reportId", "riskLevel", "emotion", "summary", "createdAt"])
            _remove_legacy_confidence_column(sheet)
            sheet.append([report.id, report.risk_level, report.emotion, report.summary, report.created_at.isoformat()])
            workbook.save(path)
    ...
    def _email_body(self, report: PsychologicalReport, case: RiskCase | None = None) -> str:
        ...
        return "\n".join(
            [
                "心理ai检测到一条高风险心理预警，请尽快安排辅导员或管理员跟进。",
                "",
