...
class PrivacyAndAssessmentTests(unittest.TestCase):
    ...
    def test_high_risk_signal_uses_hard_guard_before_model(self):
        ...
        self.assertEqual(result.summary, "检测到明确高风险表达")


class AsyncAssessmentTests(unittest.IsolatedAsyncioTestCase):
    async def test_model_outage_reaches_runtime_fail_closed_policy(self):
        with self.assertRaises(RuntimeError):
            await PsychologicalAssessmentService(ExplodingAi()).assess_async("最近压力很大")
